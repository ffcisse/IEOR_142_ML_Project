...
Build a dashboard (check out plotly/dash for example) / tool of some sorts / “deploy” your method and make it available online, allowing others (incl course staff and myself) to interact with it or explore your application, method, results, and/or data. +2.5% for a great rating, +5% for an exceptional rating. Include a link in your report to the tool (preferred) or to a recording of a demo of it.
...
I highly encourage you to use Github for this project to help you with coding collaboratively on the project as a team. Here is a guide for integrating Github in VSCode, though you can use it with any IDE. This way you can make a repository for your project code that all team members can access and contribute to. You will also almost certainly use Git in any job that requires writing or maintaining any kind of software, so if you are not familiar it is a great tool to learn. 

Finding datasets:
A great source of data is Kaggle: Your Home for Data Science, which has datasets on just about anything. You can add some filters like csv, usability, and dataset size (example) in the dataset search. The UCI ML Repository also hosts many datasets and all kinds of things. SCORE is a repository of lots of sports data.There are also some governmental challenges (some with cash prizes) from time to time posted here that come with datasets (though for some you need to submit a short application). Stanford Large Network Dataset Collection has some very large network / graph datasets, also Stanford Biomedical Network Dataset Collection , which can be a bit challenging to work with in our contexts or might require some more preprocessing but could be very interesting. There are lots and lots and lots of other data sources for pretty much anything else you can think of! 
 If a dataset you're interested in working with is prohibitively large, note that you can also just use a smaller subset of it; that is fine (e.g. just use the first 50,000 rows).
